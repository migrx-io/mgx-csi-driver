...
func setupFlags() {
	...
	flag.IntVar(&conf.NvmeCmdTimeoutSec, "nvme-cmd-timeout", 30, "Per-invocation deadline (seconds) for `nvme connect` / `nvme disconnect` shell commands; the process is killed if it has not returned within this window")
